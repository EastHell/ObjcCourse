...
//  StringTask
//
//  Created by Aleksandr on 03/04/2019.
...
#pragma mark - Level_1
    ...
    NSLog(@"\nFirst part: %@", [text substringWithRange:range]);
    
    range = NSMakeRange(text.length / 2, text.length - (text.length / 2));
    NSLog(@"\n\nSecond part: %@", [text substringWithRange:range]);
...
#pragma mark - Level_2
    ...
    NSMutableArray *arrayWithNSStringWord = [NSMutableArray array];
    for (NSString *str in array) {
        if ([str containsString:@"NSString"]) {
            [arrayWithNSStringWord addObject:str];
        }
    }
    NSLog(@"\n\n%@", arrayWithNSStringWord);
    
#pragma mark - Level_3
    
    NSMutableArray *arrayWithBackwardString = [NSMutableArray array];
    for (NSString *str in arrayWithNSStringWord) {
        NSArray *arr = [str componentsSeparatedByString:@" "];
        arr = [[arr reverseObjectEnumerator] allObjects];
        [arrayWithBackwardString addObject:[arr componentsJoinedByString:@" "]];
    }
    NSLog(@"\n\n%@", arrayWithBackwardString);
    
#pragma mark - Level_4
    
    NSMutableString *reversedText = [NSMutableString string];
    [text enumerateSubstringsInRange:NSMakeRange(0, text.length) options:NSStringEnumerationReverse | NSStringEnumerationByComposedCharacterSequences usingBlock:^(NSString * _Nullable substring, NSRange substringRange, NSRange enclosingRange, BOOL * _Nonnull stop) {
        [reversedText appendString:substring];
    }];
    text = [reversedText capitalizedString];
    reversedText = [NSMutableString string];
    [text enumerateSubstringsInRange:NSMakeRange(0, text.length) options:NSStringEnumerationReverse | NSStringEnumerationByComposedCharacterSequences usingBlock:^(NSString * _Nullable substring, NSRange substringRange, NSRange enclosingRange, BOOL * _Nonnull stop) {
        [reversedText appendString:substring];
    }];
    text = [reversedText copy];
